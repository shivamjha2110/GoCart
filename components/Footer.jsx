...
const Footer = () => {
    ...
    const linkSections = [
        ...
        },
        {
            title: "CONTACT",
            links: [
                { text: "+91 XXX-XXX-XX94", path: '/', icon: MailIcon },
                { text: "contact@gotocart.com", path: '/', icon: PhoneIcon },
                { text: "Bareilly, 243001", path: '/', icon: MapPinIcon }
            ]
        }
    ];
    ...
    const socialIcons = [
        ...
        { icon: LinkedinIcon, link: "https://www.linkedin.com/in/shivamjha21/" },
    ]
            ...
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30 text-slate-500">
                    <div>
                        ...
                        <p className="max-w-[410px] text-sm leading-relaxed">
                            Welcome to <strong>GoToCart</strong>, your ultimate destination for the latest and smartest gadgets. We bring you the best in innovation all in one place.
                        </p>
                        <div className="flex items-center gap-3 mt-5">
                            {socialIcons.map((item, i) => (
                                <Link href={item.link} key={i} className="flex items-center justify-center w-10 h-10 bg-slate-100 hover:bg-indigo-50 hover:text-indigo-600 hover:scale-110 transition-all rounded-full">
                                    <item.icon />
                                </Link>
                            ))}
                        </div>
                    ...
                    <div className="flex flex-wrap justify-between w-full md:w-[45%] gap-5 text-sm ">
                        {linkSections.map((section, index) => (
                            <div key={index}>
                                <h3 className="font-bold text-slate-800 tracking-wide mb-4">{section.title}</h3>
                                <ul className="space-y-3">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-2 group">
                                            {link.icon && <span className="group-hover:text-indigo-600 transition"><link.icon /></span>}
                                            <Link href={link.path} className="hover:text-indigo-600 hover:translate-x-1 transition-all inline-block">{link.text}</Link>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                ...
                <div className="flex justify-between items-center py-6 text-sm text-slate-500">
                    <p>Copyright 2025 © GoToCart. All Rights Reserved.</p>
